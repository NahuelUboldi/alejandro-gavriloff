...
const getFilterBtns = (categories) => {
  ...
  if(cat === state.categoryActive) {
    return acc + `<button type="button" class="btn btn-outline-primary gallery-filter-btn active">${cat}</button>`
  }
    return acc + `<button type="button" class="btn btn-outline-primary gallery-filter-btn">${cat}</button>`
  },"");
  return btns
} 
...
const getPaintingsToShow = (paintings) => {
 return paintings.reduce((acc,paint) => {
    return acc + `<img src=${paint.img.sm} alt="pintura del artista Alex Gavriloff" id="gallery-img" key=${paint.id} />`
  },"")
}
