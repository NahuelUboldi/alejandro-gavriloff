...
const convertObjectToHtmlString = (arr) => {
  return arr.reduce((acc,paint) => {
    ...
    src=${paint.img.sm} 
    id="carousel-img" 
    class=${paint.period == "1" ? "first-period" : paint.period == "2" ? "second-period" : paint.period == "3" ? "third-period" : ""}
    alt="pintura clasica impresionista del pintor argentino ruso Alejandro Gavriloff" />` 
  },"")
}

const loadPictures = (paintingsByPeriod) => {
 const firstPeriodPaintingsShufled = shuffleArray(JSON.parse(JSON.stringify(paintingsByPeriod.firstPeriod)))
 const secondPeriodPaintingsShufled = shuffleArray(JSON.parse(JSON.stringify(paintingsByPeriod.secondPeriod)))
 const thirdPeriodPaintingsShufled = shuffleArray(JSON.parse(JSON.stringify(paintingsByPeriod.thirdPeriod)))
 firstPeriodGlider.innerHTML = convertObjectToHtmlString(firstPeriodPaintingsShufled)
 secondPeriodGlider.innerHTML = convertObjectToHtmlString(secondPeriodPaintingsShufled)
 thirdPeriodGlider.innerHTML = convertObjectToHtmlString(thirdPeriodPaintingsShufled)

 //modal
 const handleClick = (e) => {
  console.log(e.target);
  if (e.target.id === 'carousel-img') {
   if (e.target.classList.contains("first-period")) showModal(modal, firstPeriodPaintingsShufled, e.target.attributes.key.value);
   if (e.target.classList.contains("second-period")) showModal(modal, secondPeriodPaintingsShufled, e.target.attributes.key.value);
   if (e.target.classList.contains("third-period")) showModal(modal, thirdPeriodPaintingsShufled, e.target.attributes.key.value);
  }
 }
